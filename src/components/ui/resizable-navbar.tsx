...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto rounded-full hidden w-full max-w-7xl flex-row items-center justify-between self-start px-4 py-2 lg:flex",
        visible && "bg-bg/20",
        className,
      )}
    >
...
export const NavItems = ({ items, className, onItemClick, onItemClickCustom, activeSection }: NavItemsProps & { activeSection?: string }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute inset-0 hidden flex-1 flex-row items-center justify-end lg:right-24 space-x-2 text-sm font-medium text-zinc-600 transition duration-200 hover:text-zinc-800 lg:flex lg:space-x-2",
        className,
      )}
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={(e) => {
            e.preventDefault();
            if (onItemClickCustom) {
              onItemClickCustom(item.link);
            } else if (onItemClick) {
              onItemClick();
            }
          }}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute right-0 top-16 z-5 h-64 flex w-32 flex-col items-start justify-start gap-4 rounded-lg bg-bg px-4  shadow-[0_0_24px_rgba(34,_42,_53,_0.06),_0_1px_1px_rgba(0,_0,_0,_0.05),_0_0_0_1px_rgba(34,_42,_53,_0.04),_0_0_4px_rgba(34,_42,_53,_0.08),_0_16px_68px_rgba(47,_48,_55,_0.05),_0_1px_0_rgba(255,_255,_255,_0.1)_inset]",
            className,
          )}
        >
          {children}
